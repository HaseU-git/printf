...
/*   Updated: 2020/12/31 14:20:46 by yhasegaw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_ux(va_list list, t_info *info)
{
	...
	|| (!info->negative_flag && info->precision >= 0))
		ft_putsame_std(' ', info->min_width, visible_len, info);
	if (!info->negative_flag && info->zero_flag && info->precision < 0)
		ft_putsame_std('0', info->min_width, visible_len, info);
	if (info->precision - check_hex_len(x, info) >= 0)
		ft_putsame_std('0', visible_len - check_hex_len(x, info), 0, info);
...
void	print_p(va_list list, t_info *info)
{
	...
	if (info->precision - check_ptr_len(ptr) >= 0)
		ft_putsame_std('0', visible_len - check_hex_len(ptr, info), 0, info);
...
void	print_di(va_list list, t_info *info)
{
	...
	if (!info->negative_flag && info->zero_flag && info->precision < 0)
		...
		+ is_negative(d), info);
	test = info->precision - count_digit(d);
	if (info->precision - count_digit(d) >= 0)
		ft_putsame_std('0', visible_len - count_digit(d), 0, info);
